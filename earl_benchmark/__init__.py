...
class EARLEnvs(object):
  ...
  def get_train_env(self, lifelong=False):
    if self._env_name == 'tabletop_manipulation':
      ...
      train_env = sawyer_peg.SawyerPegV2(reward_type=self._reward_type,
                                         reset_at_goal=self._reset_train_env_at_goal)
    elif self._env_name == 'sawyer_peg_pick_and_place':
      from earl_benchmark.envs import sawyer_peg_pick_and_place
      train_env = sawyer_peg_pick_and_place.SawyerPegPickAndPlaceV2(reward_type=self._reward_type,
                                         reset_at_goal=self._reset_train_env_at_goal)
  ...
  def get_eval_env(self):
    if self._env_name == 'tabletop_manipulation':
      ...
      from earl_benchmark.envs import sawyer_peg_pick_and_place
      eval_env = sawyer_peg_pick_and_place.SawyerPegPickAndPlaceV2(reward_type=self._reward_type)
    elif self._env_name == 'kitchen':
      from earl_benchmark.envs import kitchen
      kitchen_task = self._kwargs.get('kitchen_task', deployment_eval_config[self._env_name]['task'])  
  ...
  def get_initial_states(self, num_samples=None):
    ...
    if self._env_name == 'tabletop_manipulation':
      ...
      from earl_benchmark.envs import sawyer_peg_pick_and_place
      return sawyer_peg_pick_and_place.initial_states
  ...
  def get_goal_states(self):
    if self._env_name == 'tabletop_manipulation':
      ...
      from earl_benchmark.envs import sawyer_peg_pick_and_place
      return sawyer_peg_pick_and_place.goal_states
