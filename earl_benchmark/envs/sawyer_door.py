...
class SawyerDoorV2(SawyerDoorCloseEnvV2):
  ...
  def _get_obs(self):
    ...
    if self.add_velocity_info == 'door':
      obs = np.concatenate([
          endeff_config, obj_pos, obj_vel, self.goal,
      ])
    elif self.add_velocity_info == 'ee_door':
      obs = np.concatenate([
  ...
  def reset_goal(self, goal=None, add_noise = False):
    ...
    self.goal = goal #[ee(3), grip(1), obj(3)]
    ...
    if self.proprioceptive_only:
      # previous
      # self.sim.model.site_pos[self.model.site_name2id('goal')] = self._end_effector_goal #+ np.array([0.1, 0.1, 0]) # + np.random.uniform(size=3)*0.05
      ...
      # previous
      # self.sim.model.site_pos[self.model.site_name2id('goal')] = self._handle_goal #+ np.array([0.1, 0.1, 0]) # + np.random.uniform(size=3)*0.05
