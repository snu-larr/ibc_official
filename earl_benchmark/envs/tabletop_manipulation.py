...
class TabletopManipulation(MujocoEnv):
  ...
  def viewer_setup(self):
    ...
    self.viewer.cam.distance = 10.5 # 2.5
  ...
  def reset_goal(self, goal=None, add_noise = False): 
    ...
    self.goal = goal #[ee(2), obj(2), grip_state(2)]
    
    if self.proprioceptive_only: 
      option = 1
      self.data.site_xpos[self.model.site_name2id('hand_goal')] = np.concatenate([goal[:2], np.array([0.5])])
      if option==1: 
        self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([np.array([100.0, 100.0]), np.array([0.5])])
      elif option==2: 
        self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([goal[:2], np.array([0.5])])
      
    else:    
      self.data.site_xpos[self.model.site_name2id('hand_goal')] = np.concatenate([goal[:2], np.array([0.5])])
      self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([goal[2:4], np.array([0.5])])
  ...
  def step(self, action):
    ...
    if self.proprioceptive_only: 
      option = 1
      self.data.site_xpos[self.model.site_name2id('hand_goal')] = np.concatenate([self.goal[:2], np.array([0.5])])
      if option==1: 
        self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([np.array([100.0, 100.0]), np.array([0.5])])
      elif option==2: 
        self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([self.goal[:2], np.array([0.5])])
      
    else:
      self.data.site_xpos[self.model.site_name2id('hand_goal')] = np.concatenate([self.goal[:2], np.array([0.5])])
      self.data.site_xpos[self.model.site_name2id('obj_goal')] = np.concatenate([self.goal[2:4], np.array([0.5])])
    ...
    # self.sim.model.site_pos[self.model.site_name2id('goal')] = self._handle_goal
      if hand_pos is not None:
        for i in range(hand_pos.shape[0]):
          self.sim.model.site_pos[self.model.site_name2id('hand_'+str(i+1))] = np.concatenate([hand_pos[i], np.array([0.5])])
      if obj_pos is not None:
        for i in range(obj_pos.shape[0]):
          self.sim.model.site_pos[self.model.site_name2id('obj_'+str(i+1))] = np.concatenate([obj_pos[i], np.array([0.5])])