...
initial_ee_pos = np.array([-8.49578129e-07,  6.00251578e-01,  1.96188105e-01])

# Same as SawyerPeg but without box
class SawyerPegPickAndPlaceV2(SawyerXYZEnv):
  ...
  def __init__(self, reward_type='dense', reset_at_goal=False):
    ...
    obj_goal_low = (-0.1, 0.4, 0.01478)
    obj_goal_high = (0.3, 0.8, 0.3)
    ...
    self.obj_goal_space = Box(
      np.array(obj_goal_low),
      np.array(obj_goal_high)
    )
    self.obj_goal_height_offset = 0.01478
    ...
    self.initial_ee_pos = initial_ee_pos.copy()    
  ...
  def model_name(self):
    return os.path.join(
      os.path.dirname(os.path.realpath(__file__)), "metaworld_assets/sawyer_xyz", 'sawyer_peg_pick_and_place.xml')
    # print('fetch block is used for sawyer peg')
    # return os.path.join(
    #   os.path.dirname(os.path.realpath(__file__)), "metaworld_assets/sawyer_xyz", 'sawyer_peg_pick_and_place_fetch_block.xml')
  ...
  def get_next_goal(self):
    if not self._reset_at_goal:
      ...
      # return self.goal_states[goal_idx] # previous ver
      
      # previous
      # obj_goal = self.obj_goal_space.sample()
      obj_goal = self.initial_ee_pos + np.random.uniform(-0.15, 0.15, size=3)      
      obj_goal[2] = self.obj_goal_height_offset
      if np.random.uniform(low=0,high=1) < 0.5:
          obj_goal[2] += self.np_random.uniform(0, 0.3)
      hand_ee_goal = self.goal_states[goal_idx][:4]
      return np.concatenate([hand_ee_goal, obj_goal], axis =-1) # to make full state goal
  ...
  def reset_goal(self, goal=None, add_noise = False):
    ...
    if self.proprioceptive_only:      
      # NOTE: model.site_pos vs data.site_xpos? Use the former. do_simulation will make both equal.
      self.sim.model.site_pos[self.model.site_name2id('goal')] = np.array([-10.0, -10.0, -10.0])
      self.sim.model.site_pos[self.model.site_name2id('backward_goal')] = self._end_effector_goal      

    else:      
      self.sim.model.site_pos[self.model.site_name2id('backward_goal')] = np.array([-10.0, -10.0, -10.0])
  ...
  # override just for addressing goal visualize
  @property
  ...
  def compute_reward(self, obs, action=None):
    ...
    if self._reward_type == 'sparse':
      ...
      # mod for batch inputs
      reward = (self.is_successful(obs=obs)).astype(np.float)
  ...
  def is_successful(self, obs=None, proprioceptive_only=False):
    ...
    if self.proprioceptive_only or proprioceptive_only: # consider only gripper
      return np.linalg.norm(obs[..., :3] - obs[..., 7:10], axis =-1) <= 0.05
    else:
      return np.linalg.norm(obs[..., 4:7] - obs[..., 11:14], axis =-1) <= self.TARGET_RADIUS
  ...
  def render(self, mode='human', camera_name='topview'):
    if mode == 'human':
      ...
      return self.sim.render(
          640, 480,
          mode='offscreen',
          camera_name=camera_name # 'topview'
      ) # [::-1, :, :] 
    else:
    ...
    # self.sim.model.site_pos[self.model.site_name2id('goal')] = self._handle_goal
      if hand_pos is not None:
        for i in range(hand_pos.shape[0]):
          self.sim.model.site_pos[self.model.site_name2id('hand_'+str(i+1))] = hand_pos[i]          
      if obj_pos is not None:
        for i in range(obj_pos.shape[0]):
          self.sim.model.site_pos[self.model.site_name2id('obj_'+str(i+1))] = obj_pos[i]          
